...
#from ..network_management.transport_manager import TransportManager
...
class QuantumRouter(Node):
    ...
    def __init__(self, name, tl, memo_size=100):
...
class QKDNode(Node):
    ...
    def __init__(self, name: str, timeline: "Timeline", encoding=polarization, stack_size=5):
        ...
        self.spdcsource=SPDCSource(name + ".spdcsource", timeline,encoding_type=encoding)
        # self.transport_manager=TransportManager(self)
        # self.network_manager = NewNetworkManager(self)
        ...
        # Create for E91 protocol
        if stack_size >2:
            self.protocol_stack[2] = E91(self, name + ".E91")
    ...
    def receive_message(self, src: str, msg: "Message") -> None:
        # signal to protocol that we've received a message
        for protocol in self.protocols:
            print('protocol msg',type(protocol),msg.msg_type)
            if type(protocol) == msg.owner_type:
                protocol.received_message(src, msg)
                return
