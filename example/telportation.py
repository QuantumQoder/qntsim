...
#random.seed(0)
network_config = "/home/aman/QNTSim/QNTSim/example/teleportation.json"
...
alice='a'
bob = 'b'
print('node1 node2', alice, bob)
tm=network_topo.nodes[alice].transport_manager
tm.request(bob,start_time=5e12,size=5, end_time=10e12, priority=0 , target_fidelity= 0.5, timeout=2e12)

table=[]
print('alice memories')
for info in network_topo.nodes[alice].resource_manager.memory_manager:
    if info.state == 'ENTANGLED' or info.state == 'OCCUPIED' or info.state == 'RAW':
