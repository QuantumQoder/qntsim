...
export class AdvancedComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnDestroy(): void {
    this.diagramStorage.setAppSettingsFormDataAdvanced({
      ...
      detectorProps: this.detectorProps,
      lightSourceProps: this.lightSourceProps,
    });
  ...
  ngAfterViewInit(): void {
    this.initDiagram();
    this.updateNodes();
    const appSettingsBackup =
      this.diagramStorage.getAppSettingsFormDataAdvanced();
  ...
  linkClicked(link: any) {
    ...
    this.linksProps.attenuation =
      this.linkData[this.linksProps.keySelected].attenuation;
    this.linksProps.distance =
      this.linkData[this.linksProps.keySelected].distance;
  ...
  addNode(nodetype: string) {
    ...
    const newKey =
      this.myDiagram.model.nodeDataArray[
        this.myDiagram.model.nodeDataArray.length - 1
      ].key;
    const linkKey =
    ...
    this.myDiagram.model.addLinkData(
      this.diagramBuilder.addNewLink(linkKey, adornedPart, newNode)
    );
  ...
  drawLink(e, part) {
    ...
    if (this.linkConnection.key == -1) {
      let linkKey =
        this.myDiagram.model.linkDataArray.length > 0
          ? this.myDiagram.model.linkDataArray.at(-1).key
          : 0;
    ...
    if (this.linkConnection.from != -1 && this.linkConnection.to != -1) {
      if (
        this.diagramBuilder.isDirectLinkExists(
          this.linkConnection.from,
          this.linkConnection.to,
          this.myDiagram
        )
      ) {
  ...
  initDiagram() {
    ...
    this.myDiagram.nodeTemplate = $(
      ...
      "Auto",
      {
        ...
        click: (event, node: any) => {
          if (this.linkConnection.from !== -1) this.drawLink(event, node);
        },
      ...
      $(
        ...
        $(
          "Button",
          {
            ...
            click: (event, button: any) => {
              this.drawLink(event, button.part.adornedPart);
            }, // click on Button and then click on target node
    ...
    this.myDiagram.addDiagramListener("TextEdited", (e: any) => {
      ...
      if (nodeData && nodeData.properties) {
        ...
        const editedProperty = nodeData.properties.find(
          (prop: any) => prop.propValue.toString() === tb.text
        );
        //editedProperty)
      ...
      if (nodeData && nodeData.memory) {
        const editedProperty = nodeData.memory.find(
          (prop: any) => prop.propValue.toString() === tb.text
        );
        // //editedProperty)
