...
def __insert_seq__(lst:List, seq:List) -> Tuple[List[int], List]:
...
def __separate_seq__(indices:Sequence, lst:Sequence) -> Tuple[List, List]:
...
def insert_decoy_photons(network:QNtSim, node_index:int, num_decoy_photons:int):
...
# def insert_decoy_photons(interface:Interface, src_node:str, dst_node:str):
#     photons = interface.photons.get(src_node).get(dst_node)
#     insert_locs, new_photon_seq = __insert_seq__()