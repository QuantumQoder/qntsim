...
class QNtEmulator(QNtSim):
    def __init__(self, topology:Union[str, Dict[str, Dict[str, Any]]], messages_list:List[Dict[Tuple[str], str]], **kwds) -> None:
        ...
        self.interfaces.extend([QNtEmulator(topology=topology, messages_list=[messages], **kwds) for messages in messages_list[1:]])
    ...
    def execute(topology:Dict, app_settings:Dict):
        response = {}
        netcor = QNtEmulator(name="network", topology=topology)
