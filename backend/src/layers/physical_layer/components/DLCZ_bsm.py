...
from ...physical_layer.components.bk_memory import Memory
from ...physical_layer.components.DLCZ_memory import Memory
...
class TimeBinBSM(BSM):
    ...
    def get(self, photon):
        ...
        if res == 2:
            ...
            event =Event(int(round(early_time)), self.detectors[detector_num], "get", [])
            ...
            event = Event(int(round(late_time)), self.detectors[detector_num], "get", [])
            #self.timeline.schedule(event)
            self.timeline.schedule_counter += 1
            self.timeline.events.push(event)
        ...
        # send photons to different detectors at the early and late time
            ...
            #event = Event(int(round(late_time)), process)
            event = Event(int(round(late_time)), self.detectors[1 - detector_num], "get", [])
            #self.timeline.schedule(event)
            self.timeline.schedule_counter += 1
            self.timeline.events.push(event)
...
class SingleAtomBSM(BSM):
    ...
    def get(self, photon):
        ...
        Side Effects:
            May call get method of one of the attached detector.
        """
