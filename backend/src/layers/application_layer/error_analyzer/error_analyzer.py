...
class ErrorAnalyzer:
    ...
    def _analyse(network: Optional[Communication|Network], i: int = 0):
        ...
        logger.info(f"Avg. err. for iteration {i}: {err_prct}%")
        logger.info(f"Deviation in err. for iteration {i}: {err_sd}")
        logger.info(f"Fidelity of the message retained: {100-err_prct}%")
        logger.info(f"{info_lk}% info leaked to the attacker.")
    ...
    @classmethod
    def analyse(cls, protocol: Optional[RelayManager|ProtocolPipeline]):
