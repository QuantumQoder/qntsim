...
class Protocol:
    def __init__(self, messages_list:List[Dict[int, str]], name:str='protocol', **kwds) -> None:
        ...
        if 'label' not in kwds: self.__funcs.extend([partial(kwds.get('encode', Network.encode), msg_index=i) for i in range(1, len(messages_list[0]))])
    ...
    def __call__(self, topology:str, functions:List[partial]=None, *args: Any, **kwds: Any) -> Any:
        Network._funcs = functions if functions else self.__funcs
        start_time = time_ns()
        self.networks = [Network(**kwds, name=self.__name, topology=topology, messages=messages)(i) for i, messages in enumerate(self.messages_list)]
        mid_time = time_ns()
        logging.info('generated all networks')
        Network.execute(networks=self.networks)
        if Network._funcs==self.__funcs: self.recv_msgs_list = kwds.get('decode', Network.decode)(networks=self.networks)
