...
class Network:
    ...
    def __init__(self,
        ...
        self._initiate()
        self.get_keys(node_index=self.keys_of if hasattr(self, 'keys_of') else 0, info_state='ENTANGLED')
    ...
    def _initiate(self):
        ...
        if len(self.nodes) > 1:
            ...
            self._rectify_entanglements(label=self.label if hasattr(self, 'label') else '00')
    ...
    def _set_parameters(self):
        ...
        for key, value in data.items():
            ...
            nodes = self._net_topo.get_nodes_by_type(key) if key != 'qchannel' else self._net_topo.qchannels
            ...
            for component in nodes:
                if key == 'qchannel':
                    ...
                    if k == 'SWAP_SUCCESS_PROBABILITY':
                        component.network_manager.protocol_stack[1].set_swapping_success_rate(v)
                    else:
                        component.network_manager.protocol_stack[1].set_swapping_degradation(v)
    ...
    def _identify_quantum_states(self):
        ...
        for node in self[:-1]:  # iterate over all nodes except the last node
            for info in node.resource_manager.memory_manager:
                ...
                bell_pair_index_i = (1 - int(state[bell_pair_index_j]/state[3-bell_pair_index_j])) // 2
                self._bell_pairs[tuple(self.manager.get(key).keys)] = (bell_pair_index_i, bell_pair_index_j)
    ...
    def _initialize_photons(self):
        ...
        logging.info(f"Initialized {self.size} photons with initial states {self._initials}")
