...
def string_to_binary(messages: Dict[Tuple, str]):
    ...
    strings = [''.join('{:08b}'.format(ord(char))
                       for char in message) for message in messages.values()]
...
class Network:
    ...
    def __init__(self,
        ...
        self._bin_msgs = list(messages.values()) if all(char in '01' for message in messages.values(
        ) for char in message) else string_to_binary(messages=messages)
        ...
        if hasattr(self, 'noise'):
            self.__dict__.update({noise: ERROR_TYPE[noise].value(
                *probs) for noise, probs in self.noise.items()})
        stack = inspect.stack()
        ...
        self._initiate()
        self.get_keys(node_index=self.keys_of if hasattr(
            self, 'keys_of') else 0, info_state='ENTANGLED')
    ...
    def _initiate(self):
        ...
        self._timeline = Timeline(
            stop_time=self._stop_time, backend=self._backend)
    ...
    def _set_parameters(self):
        ...
        for key, value in data.items():
            ...
            nodes = self._net_topo.get_nodes_by_type(
                key) if key != 'qchannel' else self._net_topo.qchannels
            ...
            for component in nodes:
                if key == 'qchannel':
                    ...
                    if k == 'SWAP_SUCCESS_PROBABILITY':
                        component.network_manager.protocol_stack[1].set_swapping_success_rate(
                            v)
                    else:
                        component.network_manager.protocol_stack[1].set_swapping_degradation(
                            v)
    ...
    def _request_entanglements(self):
        ...
        for key_sequence in self.messages:
            ...
            for source_key, dest_key in itertools.pairwise(key_sequence):
                ...
                transport_manager.request(dest_node.owner.name,
                                          size=self.size,
    ...
    def _identify_quantum_states(self):
        ...
        for node in self[:-1]:  # iterate over all nodes except the last node
            for info in node.resource_manager.memory_manager:
                ...
                bell_pair_index_i = (
                    1 - int(state[bell_pair_index_j]/state[3-bell_pair_index_j])) // 2
                self._bell_pairs[tuple(self.manager.get(key).keys)] = (
                    bell_pair_index_i, bell_pair_index_j)

    def _rectify_entanglements(self, label: str):
        """
        Rectifies the entanglements of a circuit based on the Bell pairs identified.
        """
        for (key1, key2), (bell_pair_index_i, bell_pair_index_j) in self._bell_pairs.items():
            qtc = QutipCircuit(1)
    ...
    def _add_noise(self, err_type: str, qtc: QutipCircuit, keys: List[int] = None):
        ...
        match err_type:
            ...
            case 'readout':
                ...
                return model._apply_readout_error_qntsim(crc=qtc, manager=self.manager, keys=keys)
    ...
    def generate_state(self, _, state: int = 0, label: str = None):
        ...
        for keys in self.messages:
            for key in keys[1:-1]:
                for info1, info2 in zip(self._net_topo.nodes.get(key).resource_manager.memory_manager[:self.size],
                                        self._net_topo.nodes.get(key).resource_manager.memory_manager[self.size:]):
    ...
    def teleport(self, returns: Any, node_index: int = 0, msg_index: int = 0):
        ...
        for bin, info in zip(self._bin_msgs[msg_index], self[node_index].resource_manager.memory_manager):
            ...
            outputs = self._add_noise(
                err_type='readout', qtc=bsa, keys=[new_key, key])
        ...
        if hasattr(self, '_initials'):
            ...
            for info, initial in zip(node.resource_manager.memory_manager, self._initials):
                ...
                if initial//2:
                    qtc.h(0)
                qtc.measure(0)
                self._outputs.append(self._add_noise(
            ...
            for keys, value in self._corrections.items():
                if len(keys) > 1:
                    ...
                    output = self._add_noise(
                        err_type='readout', qtc=qtc, keys=[key]).get(key)
                qtc = QutipCircuit(1)
                qtc = self._add_noise(err_type='reset', qtc=qtc)
                if output:
                    if self.state:
        ...
        if hasattr(self, '_initials'):
            ...
            for bin_msg in self._bin_msgs:
                ...
                for info, initial, output in zip(node.resource_manager.memory_manager, self._initials, self._outputs):
                    bin = bin_msg[info.index] if len(
                        self._bin_msgs) > 1 else '0'
                    key = info.memory.qstate_key
                    string += str(initial % 2 ^ output.get(key) ^ int(bin))
                self._strings.append(string)
        else:
            self._strings = [''.join(str(*output.values())
                                     for output in self._outputs)]
        self.recv_msgs = {rec[1:]: ''.join(chr(int(string[j*8:-~j*8], 2)) for j in range(
            len(string)//8)) for rec, string in zip(list(self.messages)[::-1], self._strings)}
    ...
    def dump(self, returns: Any, node_name: str = '', info_state: str = ''):
        ...
        """
        logging.basicConfig(filename=self.__name+'.log', filemode='w',
                            level=logging.INFO, format='%{funcName}s %(message)s')
    ...
    def __repr__(self) -> str:
        ...
        return "\n".join([
            ...
            "\n".join([
                f"{state.keys}\t{state.state}" for state in [
                    self.manager.get(info.memory.qstate_key) for info in node.resource_manager.memory_manager
                ]
            ])
            for node in self
        ])
