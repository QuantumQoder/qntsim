...
class Protocol:
    ...
    def __call__(self, functions:List[partial]=None, *args: Any, **kwds: Any) -> Any:
        Network._funcs = functions if functions else self.funcs
        start_time = time_ns()
        self.networks = [Network(**kwds, name=self._name, messages=messages) for messages in self.messages_list]
        mid_time = time_ns()
