...
class ErrorAnalyzer:
    ...
    def _analyse(network:Network, i:int=0):
        ...
        for bin_msg, string in zip(network._bin_msgs, network._strings[::-1]):
            ...
            lk_list+=np.array([int(m)^int(lk) for m, lk in zip(bin_msg, network._lk_msg if hasattr(network, '_lk_msgs') else bin_msg)])
    ...
    def analyse(cls, protocol:Protocol):
        ...
        """
        return list(zip(*[cls._analyse(network, i=i) for i, network in enumerate(protocol, 1)]))
    ...
    def run_full_analysis(type_:int, num_iterations:int, message_length:int, **kwds):
        ...
        plt.show()
        print(f'Avg. error over all the bits for {num_iterations} iterations: {mean(bit_error)}')
        print(f'Avg. info. leakage over all the bits for {num_iterations} iterations: {mean(bit_lk)}')
        print(f'Avg. mean error over all the {num_iterations} iterations: {mean(protocol.mean_list)}')
        print(f'Avg. error deviation over all the {num_iterations} iterations: {mean(protocol.sd_list)}')
        print(f'Avg. info. leakage over all the {num_iterations} iterations: {mean(protocol.lk_list)}')
        print(f'Avg. fidelity over all the {num_iterations} iterations: {mean(protocol.fid_list)}')