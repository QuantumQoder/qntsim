...
def ip1(topology:Dict, app_settings:Dict[str, Dict[str, Any]]):
    ...
    simulator = Timeline( stop_time=app_settings.get("stop_time", math.inf), backend=app_settings.get("backend", "Qutip"))
    ...
    recv_msgs = to_characters(bin_strs=[recv_msg], _was_binary=_is_binary)
    end_time = time.time()
