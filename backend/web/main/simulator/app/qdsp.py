...
def qdsp(topology:Dict, app_settings:Dict):
    ...
    simulator = Timeline(stop_time=app_settings.get("stoptime", 1e12), backend=app_settings.get("backend", "Qutip"))
    ...
    for photon, msg in zip(photons, messages[0]): src_node.send_qubit(dst=dst_node, qubit=hwp.apply(qubit=photon) if int(msg) else photon)
    photons = dst_node.receive_qubit(src=src_node)
