    ...
    def emit(self, state_list, dst: str) -> None:
        ...
        for i, state in enumerate(state_list):
            ...
            for _ in range(num_photons):
                ...
                event = Event(time, self.owner, "send_qubit", [dst, new_photon])
    ...
    def emit(self, state_list):
        ...
        for state in state_list:
            ...
            for _ in range(num_photon_pairs):
                ...
                event0 = Event(int(round(time)), self.direct_receiver, "get", [new_photon0])
                event1 = Event(int(round(time)), self.another_receiver, "get", [new_photon1])
                #self.timeline.schedule(event0)
                #self.timeline.schedule(event1)
                self.timeline.schedule_counter += 1
                self.timeline.events.push(event0)
                self.timeline.schedule_counter += 1
                self.timeline.events.push(event1)
...
class SPDCSource2(LightSource):
    ...
    def __init__(self, owner, name, timeline, direct_receiver=None, wavelengths = [], another_receiver=None, frequency=8e7, wavelength=1550,
                 bandwidth=0, mean_photon_num=0.1, encoding_type=polarization, phase_error=0):
        super().__init__(name, timeline, frequency, wavelength, bandwidth, mean_photon_num, encoding_type, phase_error)
        # self.direct_receiver = direct_receiver
        self.owner = owner
    ...
    def emit(self):
        ...
        # Pesently assuming that light source fast enough to simultaneously feed all memories
        ...
        photon1 = Photon("", wavelength=self.wavelength, location=self,
                        encoding_type=single_atom)

        # Probablistically, set these photons to null, signifying a low probability of emission at the sources when excited
        if random.choice([0,1], p = [0.1, 0.9]):
            photon0.is_null = True
            photon1.is_null = True

        # find the next possible excite time
        period = 1e12 / self.frequency
        self.next_excite_time = self.timeline.now() + period

        # send one photon to remote node
        self.owner.send_qubit(self.middle, photon0)
