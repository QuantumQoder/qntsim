...
# ## Bob encodes his message
# 
# In the first step of the protocol, Bob (or, the sended) encodes their message by creating N number of photon batches, where the batch length is equal to the length of the message from Bob. Bob initializes each photon in the batch to one of the states from $|0\rangle, |1\rangle, |+\rangle, |-\rangle$ and, then encodes his message onto these photons through the unitaries operaties $\mathcal{I}, i\sigma_y$ for 0 and 1, respectively.
# 
# This function takes Bob's message and N as inputs, performs all the encoding operations and returns the initial_states and the circuits for all the batches, as a list.
...
# # The main() function
# 
# This is the main function which takes care of the whole protocol. The function takes the sender's and receiver's messages as input and prints what each party decoded at the end of the protocol.
# 
# It also shows the outcome results in the presence of attack, which here is depicted as a simple measrement on the channel.

# In[7]:


def main(message1="Hello", message2="World", N=5, attack=False):
    """
    Method which handles the execution of the whole protocol
    
    Parameters:
    message1 : str
        The message that needs to be teleported from party 1 to party 2
    message2 : str
        The message that needs to be teleported from party 2 to party 1
    N : int
        The number of photons in each batch
    attack : bool
        For simulating the attack scenario
    """
    
    print("Sender's message:", message1)
    print("Receiver's message:", message2)
    msg1, msg2 = [], []
    for i in message1:
    ...
    for i in range(len(msg1)):
        #print(msg1[i], msg2[i])
        initials, circuits = bob_encodes(message=msg1[i], N=N)
        if attack:
            for circ in circuits:
                circ.barrier()
                circ.measure(range(circ.num_qubits), range(circ.num_qubits))
        initial, circuit, initials, bases, counts = alice_checks(initials, circuits)
        if bob_determines(initials, msg1[i], bases, counts):
            measure, str1 = alice_encodes(initial=initial, circuit=circuit, message=msg2[i])
            str2 = bob_decodes(measure, message=msg1[i])
        else:
...
# As one can see from the above cases, that for each character in the message, the code calculates the fidelity of the channels, which is greater than 0.1 when there's no eavesdropper.
